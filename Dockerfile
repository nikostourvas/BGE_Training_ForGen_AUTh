...
# Install conda environment for snakemake
# Download and install Miniforge non-interactively
RUN apt update -qq \
	&& apt -y install curl

RUN curl -L https://github.com/conda-forge/miniforge/releases/latest/download/Miniforge3-Linux-x86_64.sh -o /tmp/Miniforge3-Linux-x86_64.sh && \
    bash /tmp/Miniforge3-Linux-x86_64.sh -b -p /opt/miniforge && \
    rm /tmp/Miniforge3-Linux-x86_64.sh

# Ensure the container uses the installed conda by updating the PATH
ENV PATH="/opt/miniforge/bin:${PATH}"

# Switch to bash so that conda activation works
SHELL ["/bin/bash", "-c"]

# Install mamba in the base environment and create the new environment
RUN curl -O https://raw.githubusercontent.com/nikostourvas/BGE_Training_ForGen_AUTh/refs/heads/main/environment.yaml && \
    mamba env create --name bge_course --file environment.yaml && \
    rm environment.yaml

# Set the new environment's bin directory in PATH so that it is used by default.
ENV PATH="/opt/miniforge/envs/bge_course/bin:${PATH}"
RUN conda init
